...
export default function About() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (aboutUs?.images.lingkunganKedai.length > 0) {
        setCurrentSlide(prev => ({
          ...prev,
          lingkungan: (prev.lingkungan + 1) % aboutUs.images.lingkunganKedai.length
        }));
      }
      if (aboutUs?.images.spotTempatDuduk.length > 0) {
        setCurrentSlide(prev => ({
          ...prev,
  ...
  const nextSlide = (type: 'lingkungan' | 'spot') => {
    if (type === 'lingkungan' && aboutUs?.images.lingkunganKedai.length > 0) {
      setCurrentSlide(prev => ({
        ...prev,
        lingkungan: (prev.lingkungan + 1) % aboutUs.images.lingkunganKedai.length
      }));
    } else if (type === 'spot' && aboutUs?.images.spotTempatDuduk.length > 0) {
      setCurrentSlide(prev => ({
        ...prev,
        spot: (prev.spot + 1) % aboutUs.images.spotTempatDuduk.length
      }));
          ...
          <div className="bg-gradient-to-br from-orange-400 to-red-400 rounded-lg p-8 text-white">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-center">
              <div>
                ...
                <p className="text-orange-100 text-lg">Tahun Pengalaman</p>
              </div>
              <div>
                <div className="text-5xl font-bold mb-2">{aboutUs?.masterChefs}</div>
                <p className="text-orange-100 text-lg">Master Chef Berpengalaman</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
          ...
          <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="relative h-80">
              {aboutUs?.images.lingkunganKedai.length > 0 ? (
                <>
                  <img
          ...
          <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="relative h-80">
              {aboutUs?.images.spotTempatDuduk.length > 0 ? (
                <>
                  <img
